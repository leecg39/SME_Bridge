...
비교 대상은 `/roadmap` 헤더의 완료 항목 수가 저장소의 모든 truthy 키를 셀지, 현재 로드맵 task 정의에 존재하는 키만 셀지이다.

- 리서치 근거: 2026년 기업승계 M&A 컨설팅 지원사업은 기초자료 작성, 기업실사, 기업가치평가 같은 실제 준비 자료를 단계별 컨설팅 범위로 안내한다. 중기부 보도자료도 M&A 시장 신뢰 형성과 승계 후 성장지원까지 전 주기 과제를 제시한다. 따라서 로드맵 화면의 완료 항목 수는 사용자가 실제로 볼 수 있는 현재 task만 기준으로 계산되어야 하며, 과거 저장 키가 남아 준비 상태를 부풀리면 안 된다.
- A안: `/roadmap` 헤더가 `Object.values(checked).filter(Boolean)`로 완료 수를 계산해 `"unknown-stale-task": true`가 남으면 실제 체크박스 1개 완료 상태가 `2/16개`처럼 과대 표시된다.
- B안: `/roadmap` 헤더도 현재 phase/task 정의에서 유효한 task 키만 기준으로 완료 수와 전체 수를 계산한다.
- 성공 기준: B안은 저장값에 현재 task 1개와 stale task 1개가 함께 있어도 `/roadmap` 헤더가 `완료 항목 1/16개`를 표시해야 한다.

| 입력 | A안 결과 | B안 결과 | 판정 |
| --- | --- | --- | --- |
| 현재 task 1개 + stale task 1개 | `2/16개`로 과대 표시 | `1/16개` 유지 | B안이 로드맵 화면 신뢰성 보강 |
| 현재 task 1개만 체크 | `1/16개` | `1/16개` | B안이 정상 동작 유지 |
| task 문구 변경 후 이전 키 잔존 | 이전 키가 완료 수에 섞임 | 현재 화면 task만 계산 | B안이 장기 운영 변경에 적합 |
...
- `/roadmap` 헤더용 순수 요약 함수가 현재 phase/task 키만 기준으로 완료 수, 전체 task 수, 문서 수를 계산한다.
- 로드맵 페이지 헤더는 이 요약 값을 사용해 stale localStorage 키를 무시한다.
- 단위 테스트와 브라우저 smoke는 stale key가 함께 있어도 헤더가 `완료 항목 1/16개`를 유지하는지 검증한다.
